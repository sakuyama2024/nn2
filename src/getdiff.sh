...
for ((i = 0; i < NUM_BLOCKS; i++)); do
    ...
    # Get the block header for the given block hash and extract the time and difficulty
    block_header=$($BITCOIN_CLI getblockheader $block_hash)
    block_time=$(echo $block_header | jq -r '.time')
    difficulty=$(echo $block_header | jq -r '.difficulty')
    ...
    if [[ $i -gt 0 ]]; then
        ...
        # Print the block height, difficulty, time, and time difference
        echo "Block Height: $block_height, Difficulty: $difficulty, Time: $readable_time, Time Difference: $time_difference seconds"
    else
        # Print the block height, difficulty, and time
        echo "Block Height: $block_height, Difficulty: $difficulty, Time: $readable_time"
    fi
